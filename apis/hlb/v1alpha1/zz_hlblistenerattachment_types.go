...
type HLBListenerAttachmentInitParameters struct {

	// Layer Protocol Negotiation (ALPN) policy to use. Valid values:
	...
	// (String) The name of the secret in AWS Secrets Manager containing the SSL server certificate. This field is required if the protocol is HTTPS. The secret should contain the certificate and private key in PEM format.
	...
	// (Boolean) If true, deletion of the listener will be disabled via the API. This prevents accidental deletion of production listeners. You must set this to false and apply the change before you can delete the listener.
	...
	// you must create a new listener to move it to a different load balancer.
	...
	// behavior is undefined if different values are assigned to different listeners with the same target group.
	...
	// (Number) The port on which the load balancer is listening. Must be between 1 and 65535. Common ports:
	...
	// (String) The protocol for connections from clients to the load balancer. Valid values:
	...
	// (String) The Amazon Resource Name (ARN) of the target group to route traffic to. The target group defines where traffic will be sent (the backend instances, IP addresses, or other resources) and how health checks are performed. The target group must be in the same region as the load balancer.
...
type HLBListenerAttachmentObservation struct {
...
type HLBListenerAttachmentParameters struct {
...
// HLBListenerAttachmentSpec defines the desired state of HLBListenerAttachment
type HLBListenerAttachmentSpec struct {
	v1.ResourceSpec `json:",inline"`
	ForProvider     HLBListenerAttachmentParameters `json:"forProvider"`
	...
	// autoscaler.
	InitProvider HLBListenerAttachmentInitParameters `json:"initProvider,omitempty"`
}

// HLBListenerAttachmentStatus defines the observed state of HLBListenerAttachment.
type HLBListenerAttachmentStatus struct {
	v1.ResourceStatus `json:",inline"`
	AtProvider        HLBListenerAttachmentObservation `json:"atProvider,omitempty"`
}
...
// HLBListenerAttachment is the Schema for the HLBListenerAttachments API. Manages an HLB Listener Attachment
...
type HLBListenerAttachment struct {
	...
	Spec   HLBListenerAttachmentSpec   `json:"spec"`
	Status HLBListenerAttachmentStatus `json:"status,omitempty"`
}

// +kubebuilder:object:root=true

// HLBListenerAttachmentList contains a list of HLBListenerAttachments
type HLBListenerAttachmentList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []HLBListenerAttachment `json:"items"`
}

// Repository type metadata.
var (
	HLBListenerAttachment_Kind             = "HLBListenerAttachment"
	HLBListenerAttachment_GroupKind        = schema.GroupKind{Group: CRDGroup, Kind: HLBListenerAttachment_Kind}.String()
	HLBListenerAttachment_KindAPIVersion   = HLBListenerAttachment_Kind + "." + CRDGroupVersion.String()
	HLBListenerAttachment_GroupVersionKind = CRDGroupVersion.WithKind(HLBListenerAttachment_Kind)
)

func init() {
	SchemeBuilder.Register(&HLBListenerAttachment{}, &HLBListenerAttachmentList{})
}
