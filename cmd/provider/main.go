...
import (
	...
	"github.com/footprint-it-solutions/provider-zonehero/config"
	"github.com/footprint-it-solutions/provider-zonehero/internal/clients"
	"github.com/footprint-it-solutions/provider-zonehero/internal/controller"
	"github.com/footprint-it-solutions/provider-zonehero/internal/features"
)

func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for ZoneHero").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-zonehero"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-zonehero",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apis.AddToScheme(mgr.GetScheme()), "Cannot add ZoneHero APIs to scheme")
	...
	kingpin.FatalIfError(controller.Setup(mgr, o), "Cannot setup ZoneHero controllers")
