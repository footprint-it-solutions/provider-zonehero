...
func main() {
	var (
		app            = kingpin.New(filepath.Base(os.Args[0]), "ZoneHero support for Crossplane.").DefaultEnvars()
	...
	if *enableChangeLogs {
		...
		clo := controller.ChangeLogOptions{
			ChangeLogger: managed.NewGRPCChangeLogger(
				...
				managed.WithProviderVersion(fmt.Sprintf("provider-zonehero:%s", version.Version))),
		}
		o.ChangeLogOptions = &clo
	}

	kingpin.FatalIfError(zonehero.Setup(mgr, o), "Cannot setup ZoneHero controllers")
